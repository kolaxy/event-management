        ...
        if all(field in event_data for field in required_fields):
            ...
            try:
                organization = Organization.objects.get(
                    id=organization_id, founder=self.request.user
                )
            except Organization.DoesNotExist:
                return Response(
                    {"message": "Invalid organization or you are not founder."},
            ...
            create_event_with_delay.apply_async(
                (data_for_celery_task,), countdown=int(CELERY_WAIT_TIME)
            )

            response_msg = (
                f"Event was accepted. Wait for {str(CELERY_WAIT_TIME)} seconds."
            )
            return Response({"message": response_msg}, status=status.HTTP_202_ACCEPTED)
        else:
            return Response(
                {"message": "organization, description, date, title fields needed"},
                status=status.HTTP_400_BAD_REQUEST,
            )
